...
# - gpuagents: GPU nodes for AI agents workloads (optional - if agents feature is needed)
...
# Set to "true" to install GPU node pools for AI agents (RAG, visual search, etc.)
enableAgentsFeature="false"
...
function validate_configuration() {
    ...
    if [[ $errors -gt 0 ]]; then
        echo ""
        echo "Please edit the configuration section at the top of this script and try again."
        exit 1
    fi
}
...
echo "  - Agents (RAG/Visual Search): ${enableAgentsFeature}"
...
if [[ $install_aks_cluster == "true" ]]; then
    ...
    #=======================================================================#
    #======== GPU Node Pool: Agents (Optional) =============================#
    #=======================================================================#
    if [[ "$enableAgentsFeature" == "true" ]]; then
        echo "Adding GPU node pool for Agents..."
        gpuAgentsExists=$(az aks nodepool show -g $rg --cluster-name $aks -n gpuagents --query "name" -o tsv 2>/dev/null || true)
        if [[ ! -z $gpuAgentsExists ]]; then
            echo "gpuagents node pool already exists. Skipping."
        else
            # Agents node pool needs 2 GPUs: 1 for inference-vl, 1 for inference-agent
            az aks nodepool add -g $rg --cluster-name $aks -n gpuagents \
                ...
                --max-pods 110
            echo "Agents GPU node pool created."
        fi
    fi
    ...
    # Configure DNS label for public IP (creates <dns-label>.<region>.cloudapp.azure.com)
    DNS_LABEL="${resourcesPrefix}"
    EXPECTED_FQDN="${DNS_LABEL}.${region}.cloudapp.azure.com"
    
    echo "Configuring DNS label..."
...
if [[ "$enableAgentsFeature" == "true" ]]; then
    echo "  - gpuagents: GPU for agents (autoscale 0-2, ${gpuVmSize})"
fi
if [[ "$enableSummarizationGpu" == "true" ]]; then
